...
describe('Image', () => {
  ...
  it('passes container props to figure element', () => {
    render(<Image {...defaultProps} data-testid="image-container" aria-label="Product image" />);
  ...
  it('handles special characters in src for id generation', () => {
    ...
    const img = screen.getByRole('img');
    const expectedId =
      'tsx-cmpnt-image-https%3A%2F%2Fexample.com%2Fimage%20with%20spaces%20%26%20symbols!.png';
    expect(img).toHaveAttribute('id', expectedId);
  });

  it('preserves imgProps alt if provided, overriding caption', () => {
    render(
      <Image {...defaultProps} caption="Caption text" imgProps={{ alt: 'Custom alt text' }} />,
    );
  ...
  it('handles both style and imgProps together', () => {
    ...
    render(<Image {...defaultProps} style={containerStyle} imgProps={imgProps} />);
